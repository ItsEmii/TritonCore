...
class UnfocusSubCommand implements FactionSubCommand
{
    ...
    public function execute(CommandSender $sender, array $args): void
    {
        ...
        if ($faction->getFocus() === null) {
            $sender->sendMessage(TextFormat::colorize('&cTu Faction no está focuseando a ninguna Faction'));
            return;
        }
        $faction->setFocus(null);
        $sender->sendMessage(TextFormat::colorize('&aTu Faction ya no está focuseando a nadie'));
    }
}